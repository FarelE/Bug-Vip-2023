...
`
let buttons = [      
                    {buttonId: `h`, buttonText: {displayText: 'MENU'}, type: 1}
                ]
                let buttonMessage = {
                    image: { url: 'https://telegra.ph/file/80b630aac4c62928f50a1.jpg' },
                    caption: `${jiren}`,
                    footer: `❦︎𝘿 𝙞 𝙘 𝙠 𝘆 B o t zシ︎Ⓥ`,
                    buttons: buttons,
                    headerType: 4
                }
...
`
let buttons = [
              {buttonId: `h`, buttonText: {displayText: 'Menu Utama'}, type: 1}
            ]
...
⭔ Url : ${anu.url}`,
            ...
            break
case 'sc':
diablobotwhatsapp.reply(`sk-IG https://www.instagram.com/dickyy_heriyanto`)
break
	    case 'donasi': case 'd': case 'd': case 'd': case 'donate': {
                diablo.sendMessage(diablobotwhatsapp.chat, { image: { url: 'https://telegra.ph/file/bf5d82353a46f752f33da.jpg' }, caption: `*Hai Kak ${diablobotwhatsapp.pushName}*\n\n DANA : 08xxxx\n\n GOPAY 083890585717\n\n SAWER AKU DONG MAS :\n\n https://saweria.co/ZeynaZeycaa` }, { quoted: diablobotwhatsapp })
            }
            break
case 'verify':  {
...
case 'jadianime': case 'toanime':
 if (!mime)return diablobotwhatsapp.reply('Send Foto Muka🗿')
diablobotwhatsapp.reply('process...')
const jadianime = require('./lib/toanim.js')
 let media2 = await quoted.download()
  let result = await jadianime(media2)
  let akhir = JSON.parse(result.extra).img_urls[0]
  diablo.sendMessage(diablobotwhatsapp.chat, { image : { url : akhir } }, { quoted : diablobotwhatsapp } )
  break
case 'tt':
 if(!text) return diablobotwhatsapp.reply(`Linknya?`)
 anu = await fetchJson(`ps://api.yanzbotzz.repl.co/api/download/tiktok?url=${text}&apikey=YanzBotz`)
...
break
case 'akses':
diablobotwhatsapp.reply(`UNTUK AKSES BOT CHAT OWNER : wa.me/6289688206739`)
break
case 'sewa':
diablobotwhatsapp.reply(`UNTUK SEWA BOT CHAT OWNER : wa.me/6289688206739`)
break
case 'masuk_ke': {
if (!itsMediablo) return diablobotwhatsapp.reply(mess.owner)
